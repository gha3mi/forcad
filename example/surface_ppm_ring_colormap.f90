...
!> Render NURBS ring surfaces to a binary PPM image using ForCAD, ForImage and ForColorMap.
program surface_ppm_ring_colormap
    ...
    real(rk)                 :: center(3), inner_radius, outer_radius
    ...
    call t%timer_start()
    center       = [0.75_rk, 0.25_rk/aspect_ratio, 0.0_rk]
    outer_radius = 0.24_rk
    ...
    call preserve_reference_midline(center(1), center(2), inner_radius, outer_radius)
    call t%timer_stop(message="Setting colors")
    deallocate(Xg)

    call t%timer_start()
    center       = [0.25_rk, 0.75_rk/aspect_ratio, 0.0_rk]
    outer_radius = 0.24_rk
    ...
    call t%timer_start()
    center       = [0.75_rk, 0.75_rk/aspect_ratio, 0.0_rk]
    outer_radius = 0.24_rk
    ...
    call image%set_pnm(&
        ...
        pixels      = px)
    call image%export_pnm("ppm/ppm_ring_colormap")
    call image%finalize()
...
contains
    ...
    !===============================================================================
    subroutine preserve_reference_midline(cx, cy, inner_radius, outer_radius)
        real(rk), intent(in) :: cx, cy, inner_radius, outer_radius
        integer :: ix, ix0, ix1, iy, offset, skip1, skip2

        iy = min(max(1, int(cy*height_aspect) + 1), height)
        ix0 = max(1, int((cx + inner_radius)*width_r) + 1)
        ix1 = min(width, int((cx + outer_radius)*width_r) + 1)
        skip1 = min(width, int((cx + 1.25_rk*inner_radius)*width_r) + 1)
        skip2 = min(width, int((cx + outer_radius)*width_r))

        do concurrent (ix = ix0:ix1) local(offset) shared(px, iy, skip1, skip2) default(none)
            if (ix == skip1 .or. ix == skip2) cycle
            offset = 3*ix - 2
            px(iy, offset  ) = 163
            px(iy, offset+1) = 98
            px(iy, offset+2) = 103
        end do
    end subroutine preserve_reference_midline
    !===============================================================================
    ...
    !===============================================================================
end program surface_ppm_ring_colormap
