...
!> Render NURBS half-ring surfaces to a binary PPM image using ForCAD, ForImage and ForColorMap.
program surface_ppm_half_ring_colormap
    ...
    center       = [0.0_rk, 0.0_rk, 0.0_rk]
    outer_radius = 0.48_rk
    inner_radius = 0.16_rk
    call shape%set_half_ring(center, inner_radius, outer_radius)
    call shape%rotate_Xc(0.0_rk, 0.0_rk, -45.0_rk)
    call shape%translate_Xc([0.75_rk, 0.25_rk/aspect_ratio, 0.0_rk])
    call shape%create(3500, 1000)
    Xg = shape%get_Xg()
    call shape%finalize()
    call t%timer_stop(message="Creating a half ring")
    ...
    center       = [0.0_rk, 0.0_rk, 0.0_rk]
    outer_radius = 0.48_rk
    inner_radius = 0.02_rk
    call shape%set_half_ring(center, inner_radius, outer_radius)
    call shape%rotate_Xc(0.0_rk, 0.0_rk, -90.0_rk)
    call shape%translate_Xc([0.25_rk, 0.75_rk/aspect_ratio, 0.0_rk])
    call shape%create(3500, 1000)
    Xg = shape%get_Xg()
    call shape%finalize()
    call t%timer_stop(message="Creating a half ring")
    ...
    center       = [0.0_rk, 0.0_rk, 0.0_rk]
    outer_radius = 0.48_rk
    inner_radius = 0.44_rk
    call shape%set_half_ring(center, inner_radius, outer_radius)
    call shape%translate_Xc([0.75_rk, 0.75_rk/aspect_ratio, 0.0_rk])
    call shape%create(3500, 1000)
    Xg = shape%get_Xg()
    call shape%finalize()
    call t%timer_stop(message="Creating a half ring")
    ...
    call image%set_pnm(&
        ...
        pixels      = px)
    call image%export_pnm("ppm/ppm_half_ring_colormap")
    call image%finalize()
...
contains
    ...
    !===============================================================================
end program surface_ppm_half_ring_colormap
