...
!! one-sided clamped, unclamped, and periodic-form vectors. This is structural:
!! periodic knot-spacing extension and control-lattice closure are supplied by
!! the caller, not inferred from the knot arrays. The active interval
...
!! working precision on the active domain. Positive degree elevation converts a
...
module forcad_nurbs_volume
    ...
    !! invalid parameters; 108 singular system or Jacobian; 109 nearest-point
    ...
    contains
        ...
        generic :: set => set1, set2, set3, set4 !!> Validate and replace the complete volume definition.
...
contains
    ...
    !! into columns. Rational input stores homogeneous \(w\mathbf P\) values and
    !! a final weight component per orthogonal line. `dir` must be in `1:3` and
    !! `Wc` must be present when `rational` is true.
    pure subroutine pack_refine_volume(Xc, Wc, nc, dir, rational, Xdir)
        ...
        integer :: n1, n2, n3, ncoord, nval, i1, i2, i3, c
            ...
            if (rational) then
                do concurrent (c = 1:ncoord, i3 = 1:n3, i2 = 1:n2, i1 = 1:n1)
                    Xdir(i1, i2 + (i3-1)*n2 + (c-1)*n2*n3) = &
                        Xc((i3-1)*n1*n2 + (i2-1)*n1 + i1, c)*Wc((i3-1)*n1*n2 + (i2-1)*n1 + i1)
                end do
                do concurrent (i3 = 1:n3, i2 = 1:n2, i1 = 1:n1)
                    Xdir(i1, i2 + (i3-1)*n2 + ncoord*n2*n3) = Wc((i3-1)*n1*n2 + (i2-1)*n1 + i1)
                end do
            else
                do concurrent (c = 1:ncoord, i3 = 1:n3, i2 = 1:n2, i1 = 1:n1)
        ...
        case (2)
            ...
            if (rational) then
                ...
                do concurrent (i3 = 1:n3, i1 = 1:n1, i2 = 1:n2)
                    Xdir(i2, i1 + (i3-1)*n1 + ncoord*n1*n3) = Wc((i3-1)*n1*n2 + (i2-1)*n1 + i1)
                end do
            else
                do concurrent (c = 1:ncoord, i3 = 1:n3, i1 = 1:n1, i2 = 1:n2)
        ...
        case (3)
            ...
            if (rational) then
                ...
                do concurrent (i2 = 1:n2, i1 = 1:n1, i3 = 1:n3)
                    Xdir(i3, i1 + (i2-1)*n1 + ncoord*n1*n2) = Wc((i3-1)*n1*n2 + (i2-1)*n1 + i1)
                end do
            else
                do concurrent (c = 1:ncoord, i2 = 1:n2, i1 = 1:n1, i3 = 1:n3)
    ...
    !! their stored weights. The caller must supply `dir` in `1:3` and nonzero
    !! homogeneous weights.
    pure subroutine unpack_refine_volume(Xdir, nc, dir, ncoord, rational, Xc, Wc)
        ...
        case (1)
            if (rational) then
                ...
                if (present(Wc)) then
                    do concurrent (i3 = 1:n3, i2 = 1:n2, i1 = 1:n1)
                        Wc((i3-1)*n1*n2 + (i2-1)*n1 + i1) = Xdir(i1, i2 + (i3-1)*n2 + ncoord*n2*n3)
                    end do
                end if
            else
                do concurrent (c = 1:ncoord, i3 = 1:n3, i2 = 1:n2, i1 = 1:n1)
        ...
        case (2)
            if (rational) then
                ...
                if (present(Wc)) then
                    do concurrent (i3 = 1:n3, i1 = 1:n1, i2 = 1:n2)
                        Wc((i3-1)*n1*n2 + (i2-1)*n1 + i1) = Xdir(i2, i1 + (i3-1)*n1 + ncoord*n1*n3)
                    end do
                end if
            else
                do concurrent (c = 1:ncoord, i3 = 1:n3, i1 = 1:n1, i2 = 1:n2)
        ...
        case (3)
            if (rational) then
                ...
                if (present(Wc)) then
                    do concurrent (i2 = 1:n2, i1 = 1:n1, i3 = 1:n3)
                        Wc((i3-1)*n1*n2 + (i2-1)*n1 + i1) = Xdir(i3, i1 + (i2-1)*n1 + ncoord*n1*n2)
                    end do
                end if
            else
                do concurrent (c = 1:ncoord, i2 = 1:n2, i1 = 1:n1, i3 = 1:n3)
    ...
    !! vectors are validated before object state changes.
    ...
    !!
    !! Valid directions are 1 through 3. The result has size `ng(dir)`;
    !! invalid or unavailable state produces an allocated empty result.
    ...
    pure subroutine insert_knots(this, dir ,Xth, r, B, Bs)
        ...
        if (rational) then
            call pack_refine_volume(this%Xc, this%Wc, nc_work, dir, .true., Xdir)
        else
        ...
        if (rational) then
            call unpack_refine_volume(Xdir, nc_work, dir, ncoord, .true., Xc_new, Wc_new)
            select case(dir)
            case(1)
    ...
    pure subroutine elevate_degree(this, dir, t, B, Bs)
        ...
        if (rational) then
            call pack_refine_volume(this%Xc, this%Wc, this%nc, dir, .true., Xdir)
        else
        ...
        if (rational) then
            call unpack_refine_volume(Xdir_new, nc_new, dir, d, .true., Xc_new, Wc_new)
            select case(dir)
            case(1)
    ...
    !! one physical volume point \(\mathbf V(\xi_{i1},\xi_{i2},\xi_{i3})\)
    !! per input row.
    ...
    pure subroutine remove_knots(this, dir ,Xth,r)
        ...
        if (rational) then
            call unpack_refine_volume(Xdir, nc_work, dir, d, .true., Xc_new, Wc_new)
            select case(dir)
            case(1)
                call this%set(knot1=knot_work, knot2=this%get_knot(2), knot3=this%get_knot(3), &
    ...
    pure subroutine nearest_point(this, point_Xg, nearest_Xg, nearest_Xt, id)
        ...
        id_ = 1
        npoint = this%ng(1)*this%ng(2)*this%ng(3)
    ...
    !!
    !! A streamed \(10^3\) seed search initializes bounded three-variable
    !! Newton iteration for
    !! \(\tfrac12\|\mathbf{V}(u,v,w)-\mathbf{x}\|^2\). A non-descent Newton
    ...
    !! minimizer. A rejected Hessian sets code 108 and reaching `maxit` sets
    !! code 109.
    !!
    ...
    impure subroutine nearest_point2(this, point_Xg, tol, maxit, nearest_Xt, nearest_Xg)
        ...
        real(rk) :: Xg(size(this%Xc,2)), residual(size(this%Xc,2)), xk(3), xkn(3)
        real(rk) :: dXg(size(this%Xc,2),3), d2Xg(size(this%Xc,2),3,3)
        integer :: k, l, i, i1, i2, i3, nseed, d, dim_, ncp, a, b
        logical :: convergenz
        real(rk) :: dist, best_dist, grad_norm, step_norm, grad_step
        ...
        do while (.not. convergenz .and. k < maxit)
            ...
            grad_norm = sqrt(grad(1)*grad(1) + grad(2)*grad(2) + grad(3)*grad(3))
            step_norm = sqrt((xk(1) - xkn(1))*(xk(1) - xkn(1)) + &
                (xk(2) - xkn(2))*(xk(2) - xkn(2)) + (xk(3) - xkn(3))*(xk(3) - xkn(3)))
            ...
            if (grad_norm <= tol .or. (k>0 .and. step_norm <= tol)) then
                ...
                if (grad_step >= 0.0_rk) then
                    ...
                    dk(2) = -grad(2)
                    dk(3) = -grad(3)
                    grad_step = -(grad(1)*grad(1) + grad(2)*grad(2) + grad(3)*grad(3))
                end if
    ...
    !! a singular Jacobian sets diagnostic code 108.
    !!
    ...
    pure subroutine ansatz(this, ie, ig, Tgc, dTgc_dXg, dV, ngauss, d2Tgc_dXg2)
        class(nurbs_volume), intent(inout) :: this
        ...
        if (present(d2Tgc_dXg2)) then
            allocate(d2Tgc_dXg2(nloc,3,3), source=0.0_rk)
            do a = 1, nloc
                do beta = 1, 3
                    do alpha = 1, 3
    ...
    pure subroutine cmp_volume(this, volume, ngauss)
        ...
        real(rk) :: inv_denom2, dV, S_d, dS1_d, dS2_d, dS3_d, dx1_d, dx2_d, dx3_d
        ...
        integer :: ngauss_(3), nelem, nquad, singular_state
        logical :: rational
        ...
        do concurrent (ie = 1: nelem) &
            local(N1, N2, N3, dN1, dN2, dN3, Xt, dXt_dXksi, bval, db1v, db2v, db3v, &
                b32, denom, ddenom1, ddenom2, ddenom3, wi, inv_denom2, dV, S_d, dS1_d, &
                ...
                j1, j2, j3, idx1, idx2, idx3, idx, d, j10, j11, j20, j21, j30, j31) &
            reduce(+:volume) &
                ...
                if (rational) then
                    ...
                    if (abs(denom) > 0.0_rk) then
                        ...
                        do d = 1, 3
                            ...
                            dx3_d = (dS3_d*denom - S_d*ddenom3)*inv_denom2
                            g11 = g11 + dx1_d*dx1_d
                            g22 = g22 + dx2_d*dx2_d
                            g33 = g33 + dx3_d*dx3_d
                            if (d == 1) then
    ...
    pure function compute_Xg_nurbs_3d(Xt, knot1, knot2, knot3, degree, nc, ng, Xc, Wc, wrap_parameters) result(Xg)
        ...
        do i = 1, ng_
#else
        do concurrent (i = 1: ng_) local(N1, N2, N3, bw, wsum, n32, j1, j2, j3, d, idx, first1, first2, first3)
#endif
            do d = 1, dim_
                Xg(i,d) = 0.0_rk
            end do
                ...
                do l3 = max(0, 1 - first3), min(degree(3), nc(3) - first3)
                    ...
                    do l2 = max(0, 1 - first2), min(degree(2), nc(2) - first2)
                        ...
                        do l1 = max(0, 1 - first1), min(degree(1), nc(1) - first1)
                            ...
                            dTgc(idx,3) = (db3v*Wc(idx) - Tgc(idx)*ddenom3)/denom
                        end do
                    end do
                end do
            end if
    ...
    pure function compute_Tgc_nurbs_3d_vector(Xt, knot1, knot2, knot3, degree, nc, ng, Wc, wrap_parameters) result(Tgc)
        ...
        do i = 1, ng_
#else
        do concurrent (i = 1: ng_) local(N1, N2, N3, wsum, n32, j1, j2, j3, idx, first1, first2, first3)
#endif
            call basis_bspline_der(map_parameter(Xt(i,1), knot1, nc(1), degree(1), wrap_parameters_(1)), &
                knot1, nc(1), degree(1), 0, first1, N1)
            call basis_bspline_der(map_parameter(Xt(i,2), knot2, nc(2), degree(2), wrap_parameters_(2)), &
            ...
            if (abs(wsum) > wtol) then
                do j3 = max(0, 1 - first3), min(degree(3), nc(3) - first3)
                    do j2 = max(0, 1 - first2), min(degree(2), nc(2) - first2)
                        ...
                        do j1 = max(0, 1 - first1), min(degree(1), nc(1) - first1)
                            idx = ((first3+j3-1)*nc(2) + first2+j2-1)*nc(1) + first1 + j1
                            Tgc(i,idx) = n32*N1(j1)*Wc(idx)/wsum
                        end do
                    end do
                end do
            end if
        ...
        if (present(elem)) then
            ...
            if (local_weights) then
                do j1 = 1, nloc
                    ...
                    wsum = wsum + N3(a3)*N2(a2)*N1(a1)*Wc(j1)
                end do
            else
                do j3 = max(0, 1 - first3), min(degree(3), nc(3) - first3)
                    do j2 = max(0, 1 - first2), min(degree(2), nc(2) - first2)
    ...
    !! removal fixes projective scale at geometric mean weight one. In exact
    !! arithmetic exponentiation preserves strict positivity; machine overflow
    !! or underflow is not checked. Damping is reduced geometrically, without a
    !! trial-step acceptance test or line search.
    !!
    ...
    !! `maxit>0`, `tol>=0`, and nonnegative regularization/damping values. These
    ...
    pure subroutine lsq_fit_nurbs(this, Xt, Xdata, ndata, maxit, tol, lambda_xc, mu0, reg_logw)
        ...
        real(rk) :: bval, cost_prev, cost_now, cost_sum, damp, denom, epss, ha, hb, n32a, n32b
                                ...
                                do b3 = 0, this%degree(3)
                                    ...
                                    do b2 = 0, this%degree(2)
                                        ...
                                        do b1 = 0, this%degree(1)
                                            ...
                                            ib = ((first3+b3-1)*this%nc(2) + first2+b2-1)*this%nc(1) + first1 + b1
                                            if (ia >= ib) then
                                                hb = n32b*N1(b1)*this%Wc(ib)*(this%Xc(ib,k) - Cg(k))/denom
