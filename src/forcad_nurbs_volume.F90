...
contains
    ...
    pure subroutine lsq_fit_nurbs(this, Xt, Xdata, ndata, maxit, tol, lambda_xc, mu0, reg_logw)
        ...
        if (this%nc(1) > ndata(1)) then
            call this%err%set(1, 106,'forcad_nurbs_volume','Too few samples in dir-1.','lsq_fit_nurbs','Use nc(1) <= ndata(1).')
            return
        end if
        if (this%nc(2) > ndata(2)) then
            call this%err%set(1, 106,'forcad_nurbs_volume','Too few samples in dir-2.','lsq_fit_nurbs','Use nc(2) <= ndata(2).')
            return
        end if
        if (this%nc(3) > ndata(3)) then
            call this%err%set(1, 106,'forcad_nurbs_volume','Too few samples in dir-3.','lsq_fit_nurbs','Use nc(3) <= ndata(3).')
            return
        end if
        if (n <= 0 .or. ncp < 2) then
            call this%err%set(1, 106,'forcad_nurbs_volume','Invalid sizes for LSQ fitting.','lsq_fit_nurbs','Check ndata and nc.')
            return
        end if
...
#endif
            Bmat(i,:) = kron( &
                basis_bspline(Xt(i,3), this%knot3, this%nc(3), this%degree(3)), &
                kron( basis_bspline(Xt(i,2), this%knot2, this%nc(2), this%degree(2)), &
                    basis_bspline(Xt(i,1), this%knot1, this%nc(1), this%degree(1)) ) )
        end do
