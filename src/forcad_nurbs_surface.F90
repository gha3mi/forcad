...
!! periodic-form vectors. This is a structural statement: periodic knot-spacing
!! extension and control-net closure are supplied by the caller, not inferred
!! from the knot array. Its active interval is
...
!! precision on the active domain. Positive degree elevation converts a
!! non-clamped directional representation to an equivalent active-domain
!! open-clamped representation and does not retain periodic seam constraints.
...
module forcad_nurbs_surface
    ...
    use forcad_utils, only: basis_bspline, elemConn_C0, kron, ndgrid, compute_multiplicity, compute_knot_vector, &
        ...
        tensor_basis_derivative_local, tensor_basis_derivatives2_local, map_parameter
    ...
    contains
        ...
        generic :: set => set1, set2, set3, set4 !!> Validate and replace the complete surface definition.
...
contains
    ...
    !! Rational input is converted to homogeneous form by storing \(w\mathbf P\)
    !! followed by one weight column per orthogonal control line. `dir` must be
    !! 1 or 2 and `Wc` must be present when `rational` is true.
    pure subroutine pack_refine_surface(Xc, Wc, nc, dir, rational, Xdir)
        ...
        integer :: n1, n2, ncoord, nval, i1, i2, c
        ...
        case (1)
            ...
            if (rational) then
                do concurrent (c = 1:ncoord, i2 = 1:n2, i1 = 1:n1)
                    Xdir(i1, i2 + (c-1)*n2) = Xc((i2-1)*n1+i1, c)*Wc((i2-1)*n1+i1)
                end do
                do concurrent (i2 = 1:n2, i1 = 1:n1)
                    Xdir(i1, i2 + ncoord*n2) = Wc((i2-1)*n1+i1)
                end do
            else
                do concurrent (c = 1:ncoord, i2 = 1:n2, i1 = 1:n1)
        ...
        case (2)
            ...
            if (rational) then
                do concurrent (c = 1:ncoord, i1 = 1:n1, i2 = 1:n2)
                    Xdir(i2, i1 + (c-1)*n1) = Xc((i2-1)*n1+i1, c)*Wc((i2-1)*n1+i1)
                end do
                do concurrent (i1 = 1:n1, i2 = 1:n2)
                    Xdir(i2, i1 + ncoord*n1) = Wc((i2-1)*n1+i1)
                end do
            else
                do concurrent (c = 1:ncoord, i1 = 1:n1, i2 = 1:n2)
    ...
    !! \]
    !!
    !! Remaining columns do not affect the parameterization. The cache receives
    !! one physical surface point \(\mathbf S(u_i,v_i)\) per input row.
    ...
    !! Rational rows are dehomogenized by their stored weights. The caller must
    !! provide `dir` in `1:2` and nonzero homogeneous weights.
    pure subroutine unpack_refine_surface(Xdir, nc, dir, ncoord, rational, Xc, Wc)
        ...
        case (1)
            if (rational) then
                ...
                if (present(Wc)) then
                    do concurrent (i2 = 1:n2, i1 = 1:n1)
                        Wc((i2-1)*n1+i1) = Xdir(i1, i2 + ncoord*n2)
                    end do
                end if
            else
                do concurrent (c = 1:ncoord, i2 = 1:n2, i1 = 1:n1)
        ...
        case (2)
            if (rational) then
                ...
                if (present(Wc)) then
                    do concurrent (i1 = 1:n1, i2 = 1:n2)
                        Wc((i2-1)*n1+i1) = Xdir(i2, i1 + ncoord*n1)
                    end do
                end if
            else
                do concurrent (c = 1:ncoord, i1 = 1:n1, i2 = 1:n2)
    ...
    !! validated before object state changes.
    ...
    !!
    !! Valid directions are 1 and 2. The result has size `ng(dir)`; an
    !! invalid direction, absent cache, or active diagnostic produces an
    !! allocated empty result.
    ...
    !!
    !! The result is zero before sampling and while a diagnostic is active.
    ...
    !! Each directional wrapping option is written to both the corresponding
    !! IGES closed-direction and periodic-direction property. ForCAD does not
    !! independently verify control-net closure or periodic knot extension
    !! before setting those properties. The polynomial/rational property follows
    ...
    impure subroutine export_iges(this, filename)
        ...
        U = real([knot_start(this%knot1, this%nc(1), this%degree(1)), &
            knot_end(this%knot1, this%nc(1), this%degree(1))], kind=wp)
        V = real([knot_start(this%knot2, this%nc(2), this%degree(2)), &
            knot_end(this%knot2, this%nc(2), this%degree(2))], kind=wp)
        ...
        surf128%M1          = M1
        surf128%M2          = M2
        surf128%PROP1       = merge(1,0,this%get_parameter_wrapping(1))
        surf128%PROP2       = merge(1,0,this%get_parameter_wrapping(2))
        surf128%PROP3       = prop3
        surf128%PROP4       = merge(1,0,this%get_parameter_wrapping(1))
        surf128%PROP5       = merge(1,0,this%get_parameter_wrapping(2))
    ...
    !! selected direction. Let \([a,b]\) be its current active interval and
    !! \(\tau\) the value returned by `knot_tolerance`. Writing \(u_i\) for
    ...
    pure subroutine insert_knots(this, dir, Xth, r, B, Bs)
        ...
        integer,  contiguous,             intent(in)    :: r(:)
            !! Nonnegative insertion counts; final multiplicity is at most `degree(dir)+1`.
        ...
        if (rational) then
            call pack_refine_surface(this%Xc, this%Wc, nc_work, dir, .true., Xdir)
        else
        ...
        do i = 1, size(Xth)
            ...
            if (s + r(i) > this%degree(dir) + 1) then
                call this%err%set(&
                    code       = 100,&
                    severity   = 1,&
                    category   = "forcad_nurbs_surface",&
                    ...
                    location   = "insert_knots",&
                    suggestion = "Use r so existing plus inserted multiplicity is <= degree+1.")
                return
            end if
        ...
        if (rational) then
            call unpack_refine_surface(Xdir, nc_work, dir, ncoord, .true., Xc_new, Wc_new)
            if (dir == 1) then
    ...
    !! For `t>0`, non-clamped input in the selected direction is converted to an
    !! equivalent open-clamped representation on its active interval; exterior
    !! extension knots and periodic seam constraints in that direction are not
    !! retained. Other tensor directions are unchanged.
    !!
    ...
    pure subroutine elevate_degree(this, dir, t, B, Bs)
        ...
        if (rational) then
            call pack_refine_surface(this%Xc, this%Wc, this%nc, dir, .true., Xdir)
        else
        ...
        if (rational) then
            call unpack_refine_surface(Xdir_new, nc_new, dir, d, .true., Xc_new, Wc_new)
            if (dir == 1) then
    ...
    pure subroutine remove_knots(this, dir ,Xth,r)
        ...
        if (rational) then
            call unpack_refine_surface(Xdir, nc_work, dir, d, .true., Xc_new, Wc_new)
            if (dir == 1) then
                call this%set(knot1=knot_work, knot2=this%get_knot(2), Xc=Xc_new, Wc=Wc_new, &
                    degree=this%degree, wrap_parameters=this%wrap_parameters)
            else
    ...
    !!
    !! A streamed \(10\times10\) seed search initializes bounded two-variable
    !! Newton iteration for
    !! \(\tfrac12\|\mathbf{S}(u,v)-\mathbf{x}\|^2\). A non-descent Newton
    !! direction is replaced by the negative gradient. The line search attempts
    !! at most 50 feasible Armijo reductions; after that cap, its final feasible
    !! step can be used without a sufficient-decrease certificate. Termination
    !! uses an absolute gradient norm, an absolute parameter-step norm, or a
    !! numerically blocked feasible step. The latter does not test the projected
    !! gradient or certify the box-constrained KKT conditions. The result is
    !! therefore a bounded local candidate, not a certified local or global
    !! minimizer. A rejected Hessian sets code 108 and exhausting `maxit` sets
    !! code 109.
    !!
    ...
    impure subroutine nearest_point2(this, point_Xg, tol, maxit, nearest_Xt, nearest_Xg)
        ...
        real(rk), intent(in) :: tol
            !! Nonnegative absolute gradient-norm or parameter-step tolerance; not internally validated.
        ...
        real(rk) :: Xg(size(this%Xc,2)), residual(size(this%Xc,2)), xk(2), xkn(2)
        real(rk) :: dXg(size(this%Xc,2),2), d2Xg(size(this%Xc,2),2,2)
        integer :: k, l, i, i1, i2, nseed, d, dim_, ncp
        logical :: convergenz
        real(rk) :: dist, best_dist, detH, grad_norm, step_norm, grad_step
        ...
        do while (.not. convergenz .and. k < maxit)
            ...
            grad_norm = sqrt(grad(1)*grad(1) + grad(2)*grad(2))
            step_norm = sqrt((xk(1) - xkn(1))*(xk(1) - xkn(1)) + (xk(2) - xkn(2))*(xk(2) - xkn(2)))

            if (grad_norm <= tol .or. (k>0 .and. step_norm <= tol)) then
                convergenz  = .true.
                nearest_Xt  = xk
                ...
                if (grad_step >= 0.0_rk) then
                    dk(1) = -grad(1)
                    dk(2) = -grad(2)
                    grad_step = -(grad(1)*grad(1) + grad(2)*grad(2))
                end if
                ...
                do
                    ...
                    if (obj_trial <= obj + alphak*beta*grad_step) exit
    ...
    !! or rank-deficient metric sets diagnostic code 108.
    !!
    ...
    pure subroutine ansatz(this, ie, ig, Tgc, dTgc_dXg, dA, ngauss, d2Tgc_dXg2)
        class(nurbs_surface), intent(inout) :: this
        ...
        if (present(d2Tgc_dXg2)) then
            ...
            allocate(d2Tgc_dXg2(nloc,dim_,dim_), source=0.0_rk)
            do a = 1, nloc
                do beta = 1, 2
                    do alpha = 1, 2
    ...
    pure subroutine cmp_area(this, area, ngauss)
        ...
        real(rk) :: Xt(2), dXt_dXksi(2)
        real(rk) :: bval, db1v, db2v, denom, ddenom1, ddenom2, wi, inv_denom2
        ...
        integer :: ngauss_(2), nelem, nquad, singular_state
        logical :: rational
        ...
        do concurrent (ie = 1: nelem) &
            local(N1, N2, dN1, dN2, Xt, dXt_dXksi, bval, db1v, db2v, denom, ddenom1, &
                ddenom2, wi, inv_denom2, dA, S_d, dS1_d, dS2_d, dx1_d, dx2_d, dx11, &
                dx12, dx21, dx22, detA, detG, g11, g12, g22, tol, ig, e1, e2, first1, &
                first2, j1, j2, idx1, idx2, idx, d, j10, j11, j20, j21) &
            reduce(+:area) &
            ...
            do ig = 1, nquad
                ...
                if (rational) then
                    ...
                    if (abs(denom) > 0.0_rk) then
                        inv_denom2 = 1.0_rk/(denom*denom)
                        do d = 1, dim_
                            S_d = 0.0_rk
                            dS1_d = 0.0_rk
                            dS2_d = 0.0_rk
                            ...
                            dx1_d = (dS1_d*denom - S_d*ddenom1)*inv_denom2
                            dx2_d = (dS2_d*denom - S_d*ddenom2)*inv_denom2
                            g11 = g11 + dx1_d*dx1_d
                            g12 = g12 + dx1_d*dx2_d
                            g22 = g22 + dx2_d*dx2_d
                            if (d == 1) then
    ...
    pure function compute_Xg_nurbs_2d(Xt, knot1, knot2, degree, nc, ng, Xc, Wc, wrap_parameters) result(Xg)
        ...
        do i = 1, ng_
#else
        do concurrent (i = 1: ng_) local(N1, N2, bw, wsum, j1, j2, d, idx, first1, first2)
#endif
            do d = 1, dim_
                Xg(i,d) = 0.0_rk
            end do
        ...
        if (.not. present(elem)) then
            ...
            do l2 = max(0, 1 - first2), min(degree(2), nc(2) - first2)
                ...
                do l1 = max(0, 1 - first1), min(degree(1), nc(1) - first1)
                    ...
                    db1v = N2(l2)*dN1(l1)
                    db2v = dN2(l2)*N1(l1)
                    denom = denom + bval*Wc(idx)
                    ddenom1 = ddenom1 + db1v*Wc(idx)
                    ddenom2 = ddenom2 + db2v*Wc(idx)
                end do
            end do
            if (abs(denom) > wtol) then
                do l2 = max(0, 1 - first2), min(degree(2), nc(2) - first2)
                    ...
                    do l1 = max(0, 1 - first1), min(degree(1), nc(1) - first1)
                        ...
                        db1v = N2(l2)*dN1(l1)
                        db2v = dN2(l2)*N1(l1)
                        Tgc(idx) = bval*Wc(idx)/denom
                        dTgc(idx,1) = (db1v*Wc(idx) - Tgc(idx)*ddenom1)/denom
                        dTgc(idx,2) = (db2v*Wc(idx) - Tgc(idx)*ddenom2)/denom
                    end do
                end do
            end if
        else
    ...
    pure subroutine compute_d2Tgc_nurbs_2d_vector(Xt, knot1, knot2, degree, nc, ng, Wc, d2Tgc, dTgc, Tgc, wrap_parameters)
        ...
        do concurrent (i = 1: ng_) local(N1, N2, dN1, dN2, d2N1, d2N2, denom, dden1, dden2, &
            ...
            j1, j2, idx1, idx2, idx, first1, first2)
#endif
            call basis_bspline_der(map_parameter(Xt(i,1), knot1, nc(1), degree(1), wrap_parameters_(1)), &
    ...
    pure function compute_Tgc_nurbs_2d_vector(Xt, knot1, knot2, degree, nc, ng, Wc, wrap_parameters) result(Tgc)
        ...
        real(rk) :: N1(0:degree(1)), N2(0:degree(2)), wsum, wtol
        ...
        do i = 1, ng_
#else
        do concurrent (i = 1: ng_) local(N1, N2, wsum, j1, j2, idx, first1, first2)
#endif
            call basis_bspline_der(map_parameter(Xt(i,1), knot1, nc(1), degree(1), wrap_parameters_(1)), &
                knot1, nc(1), degree(1), 0, first1, N1)
            call basis_bspline_der(map_parameter(Xt(i,2), knot2, nc(2), degree(2), wrap_parameters_(2)), &
            ...
            if (abs(wsum) > wtol) then
                do j2 = max(0, 1 - first2), min(degree(2), nc(2) - first2)
                    do j1 = max(0, 1 - first1), min(degree(1), nc(1) - first1)
                        idx = (first2+j2-1)*nc(1) + first1 + j1
                        Tgc(i,idx) = N2(j2)*N1(j1)*Wc(idx)/wsum
                    end do
                end do
            end if
        end do
        ...
        if (present(elem)) then
            ...
            if (abs(wsum) > wtol) then
                do j2 = max(0, 1 - first2), min(degree(2), nc(2) - first2)
                    do j1 = max(0, 1 - first1), min(degree(1), nc(1) - first1)
                        idx = (first2+j2-1)*nc(1) + first1 + j1
                        Tgc(idx) = N2(j2)*N1(j1)*Wc(idx)/wsum
                    end do
                end do
            end if
        end if
    ...
    !! one. In exact arithmetic exponentiation preserves strict positivity;
    !! machine overflow or underflow is not checked. Damping is reduced
    !! geometrically, without a trial-step acceptance test or line search.
    !!
    ...
    !! \]
    !!
    !! This is not an RMS residual. Iteration stops when
    !! \(\rho_{old}-\rho\le\mathtt{tol}\max(1,\rho_{old})\), so an increase
    !! also terminates a run with nonnegative `tol`. Reaching `maxit` is not an
    !! error; the last iterate is retained. Only controls, weights, and
    !! rational-state metadata are updated; cached `Xg` is not recomputed.
    ...
    !! `tol>=0`, and nonnegative regularization/damping values. These
    !! preconditions are not all validated, and the objective is nonconvex.
    !!
    ...
    pure subroutine lsq_fit_nurbs(this, Xt, Xdata, ndata, maxit, tol, lambda_xc, mu0, reg_logw)
        ...
        real(rk), intent(in),  optional     :: tol
            !! Optional nonnegative relative decrease tolerance for the documented scaled residual norm.
        ...
        real(rk) :: bval, cost_prev, cost_now, cost_sum, damp, denom, epss, ha, hb, resid, ta, tb
                ...
                do k = 1, dim_
                    ...
                    do a2 = 0, this%degree(2)
                        ...
                        do a1 = 0, this%degree(1)
                            ...
                            ia = (first2+a2-1)*this%nc(1) + first1 + a1
                            bval = N2(a2)*N1(a1)
                            ha = bval*this%Wc(ia)*(this%Xc(ia,k) - Cg(k))/denom
                            ...
                            do b2 = 0, this%degree(2)
                                ...
                                do b1 = 0, this%degree(1)
                                    ...
                                    ib = (first2+b2-1)*this%nc(1) + first1 + b1
                                    if (ia >= ib) then
                                        hb = N2(b2)*N1(b1)*this%Wc(ib)*(this%Xc(ib,k) - Cg(k))/denom
