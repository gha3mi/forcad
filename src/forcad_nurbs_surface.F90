...
contains
    ...
    pure subroutine insert_knots(this, dir, Xth, r, B, Bs)
        ...
        if (present(B) .or. present(Bs)) then
            block
                ...
                integer :: nc1, nc2, n1_new, mS, nS, irow, jcol, i1, j1, i2, i2_old, i2_new, ii, c
                nc1 = this%nc(1)
                nc2 = this%nc(2)
                ...
                case (1)
                    ...
                    if (this%is_rational()) then
                    do concurrent (i2=0:nc2-1, j1=1:nc_old(1), i1=1:n1_new, A1(i1,j1) /= 0.0_rk) local(irow,jcol)
                        irow = i2*n1_new   + i1
                        jcol = i2*nc_old(1)+ j1
                ...
                case (2)
                    ...
                    allocate(S_loc(mS,nS), source=0.0_rk)
                    if (this%is_rational()) then
                    do concurrent (i2_old=1:nc_old(2), i2_new=1:n1_new, ii=0:nc1-1, A1(i2_new,i2_old) /= 0.0_rk) local(irow,jcol)
                        irow = ii + 1 + (i2_new-1)*nc1
                        jcol = ii + 1 + (i2_old-1)*nc1
    ...
    pure subroutine elevate_degree(this, dir, t, B, Bs)
        ...
        integer :: nc_old(2), dim, ncp_old, mS, nS, irow, jcol, c
        ...
        if (present(B) .or. present(Bs)) then
            ...
            case(1)
                ...
                allocate(S_loc(mS,nS), source=0.0_rk)
                if (this%is_rational()) then
                    do concurrent (i2=0:this%nc(2)-1, j1=1:nc_old(1), i1=1:this%nc(1), Tdir(i1,j1) /= 0.0_rk) local(irow,jcol)
                    irow = i2*this%nc(1) + i1
                    jcol = i2*nc_old(1)  + j1
            ...
            case(2)
                ...
                allocate(S_loc(mS,nS), source=0.0_rk)
                if (this%is_rational()) then
                    do concurrent (i2_old=1:nc_old(2), i2_new=1:this%nc(2), ii=0:this%nc(1)-1, Tdir(i2_new,i2_old) /= 0.0_rk) local(irow,jcol)
                    irow = ii + 1 + (i2_new-1)*this%nc(1)
                    jcol = ii + 1 + (i2_old-1)*this%nc(1)
    ...
    pure subroutine compute_dTgc_nurbs_2d_vector(Xt, knot1, knot2, degree, nc, ng, Wc, dTgc, Tgc)
        ...
        allocate(dTgc(ng_, nc(1)*nc(2), 2), Tgc(ng_, nc(1)*nc(2)))
#if defined(__NVCOMPILER) || defined(__GFORTRAN__)
        do i = 1, ng_
#else
        do concurrent (i = 1: ng_) local(B1, B2, dB1, dB2, Bi, dBi)
#endif
    ...
    pure subroutine compute_dTgc_bspline_2d_vector(Xt, knot1, knot2, degree, nc, ng, dTgc, Tgc)
        ...
        do i = 1, ng_
#else
        do concurrent (i = 1: ng_) local(B1, B2, dB1, dB2)
#endif
            call basis_bspline_der(Xt(i,1), knot1, nc(1), degree(1), dB1, B1)
            call basis_bspline_der(Xt(i,2), knot2, nc(2), degree(2), dB2, B2)
            Tgc(i,:) = kron(B2, B1)
    ...
    pure subroutine compute_d2Tgc_nurbs_2d_vector(Xt, knot1, knot2, degree, nc, ng, Wc, d2Tgc, dTgc, Tgc)
        ...
        allocate(d2Tgc(ng_, 2*nc(1)*nc(2), 2))

        ! allocate(Bi(nc(1)*nc(2)), dBi(nc(1)*nc(2), 2), d2Bi(2*nc(1)*nc(2), 2))

        allocate(Tgci(nc(1)*nc(2)), dTgci(nc(1)*nc(2)))
        allocate(Tgc(ng_, nc(1)*nc(2)), dTgc(ng_, nc(1)*nc(2), 2))
#if defined(__NVCOMPILER) || defined(__GFORTRAN__)
        do i = 1, ng_
#else
        do concurrent (i = 1: ng_) local(B1, B2, dB1, dB2, Bi, dBi, d2Bi)
#endif
    ...
    pure subroutine compute_d2Tgc_bspline_2d_vector(Xt, knot1, knot2, degree, nc, ng, d2Tgc, dTgc, Tgc)
        ...
        allocate(d2Tgc(ng_, 2*nc(1)*nc(2), 2))
        allocate(dTgc(ng_, nc(1)*nc(2), 2))
        allocate(Tgc(ng_, nc(1)*nc(2)))
#if defined(__NVCOMPILER) || defined(__GFORTRAN__)
        do i = 1, ng_
#else
        do concurrent (i = 1: ng_) local(B1, B2, dB1, dB2, d2B1, d2B2)
#endif
            call basis_bspline_2der(Xt(i,1), knot1, nc(1), degree(1), d2B1, dB1, B1)
            call basis_bspline_2der(Xt(i,2), knot2, nc(2), degree(2), d2B2, dB2, B2)
