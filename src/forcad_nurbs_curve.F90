...
!! periodic-form vectors. "Periodic-form" here means a structurally valid knot
!! vector with the required exterior knots; ForCAD does not infer periodic knot
!! spacing or control-point closure. Parameter wrapping is an evaluation option
!! and does not by itself impose periodic geometry.
!!
...
!! precision on the active domain. For positive degree elevation, a non-clamped
!! representation is converted to an equivalent active-domain open-clamped
!! representation; exterior extension knots and periodic seam constraints are
!! not retained. For curve integration,
!!
!! \[
...
module forcad_nurbs_curve
    ...
    use forcad_utils, only: basis_bspline, elemConn_C0, ndgrid, compute_multiplicity, compute_knot_vector, basis_bspline_der,&
        ...
        tensor_basis_derivatives2_local, map_parameter
    ...
    !! system or mapping; and 109 nearest-point nonconvergence.
    !!
    ...
    contains
        ...
        generic :: set => set1, set1a, set2, set3, set4 !!> Validate and replace the complete curve definition.
...
contains
    ...
    !! replaces the cached geometry points with \(\mathbf C(u_i)\); optional
    !! connectivity replaces only their visualization connectivity.
    ...
    !! `get_parameter_wrapping()` is written to both the IGES closed-curve and
    !! periodic-curve properties. ForCAD does not independently verify control-
    !! point closure or periodic knot extension before setting those properties.
    ...
    impure subroutine export_iges(this, filename)
        ...
        curve126%M           = M
        curve126%PROP1       = 0
        curve126%PROP2       = merge(1,0,this%get_parameter_wrapping())
        curve126%PROP3       = prop3
        curve126%PROP4       = merge(1,0,this%get_parameter_wrapping())
    ...
    !! `degree+1` represents a \(C^{-1}\) break. Let \([a,b]\) be the current
    ...
    pure subroutine insert_knots(this, Xth, r, B, Bs)
        ...
        integer :: i, j, k, s, n_new, ncoord, n_old, mS, nS, c, nc_work
        real(rk) :: lo, hi, ktol
        ...
        if (this%is_rational()) then
            ...
            do concurrent (j = 1: size(this%Xc,1))
                Xcw(j,1:ncoord) = this%Xc(j,1:ncoord) * this%Wc(j)
            end do
            Xcw(:,ncoord+1) = this%Wc(:)
            knot_work = this%knot
            nc_work = size(Xcw,1)
            ...
            do i = 1, size(Xth)
                ...
                if (present(B) .or. present(Bs)) then
                    ...
                    call insert_knot_A_5_1(this%degree, knot_work, Xcw, Xth(i), k, s, r(i), n_new, knot_new, Xcw_new)
                end if
            ...
            do i = 1, size(Xth)
                ...
                if (present(B) .or. present(Bs)) then
                    ...
                    call insert_knot_A_5_1(this%degree, knot_work, Xc, Xth(i), k, s, r(i), n_new, knot_new, Xcw_new)
                end if
    ...
    !! transformed homogeneously. For `t>0`, non-clamped input is converted to
    !! an equivalent open-clamped representation on the active interval;
    !! exterior extension knots and periodic seam constraints are not retained.
    ...
    pure subroutine elevate_degree(this, t, B, Bs)
        ...
        integer :: ncoord, mS, nS, c, i, j, nc_new
        ...
        if (this%is_rational()) then
            ...
            do concurrent (i = 1:size(this%Xc,1))
                Xcw(i,1:ncoord) = this%Xc(i,1:ncoord) * this%Wc(i)
            end do
            Xcw(:,ncoord+1) = this%Wc(:)
    ...
    pure subroutine remove_knots(this,Xth,r)
        ...
        integer :: k, i, s, d, j, nc_new, t, nc_work
        real(rk) :: lo, hi, ktol
    ...
    !! Each row contains `degree+1` one-based control indices.
    ...
    !!
    !! Ten streamed parameter seeds choose an initial point. Bounded Newton
    !! iterations minimize \(\tfrac12\|\mathbf{C}(u)-\mathbf{x}\|^2\); a
    !! non-descent Newton direction is replaced by the negative gradient. The
    !! line search attempts at most 50 feasible Armijo reductions. At that cap
    !! the last feasible step can be accepted without a sufficient-decrease
    !! certificate. Termination occurs for the absolute gradient test, the
    !! absolute parameter-step test, or a numerically blocked feasible step;
    !! the last case does not by itself certify constrained stationarity.
    !! Consequently the output is a bounded local candidate, not a certified
    !! local or global minimizer. Failure to reach a stopping test within
    !! `maxit` sets code 109; a rejected scalar Hessian sets code 108. Once an
    !! iteration has been initialized, outputs contain its last defined iterate.
    !!
    ...
    impure subroutine nearest_point2(this, point_Xg, tol, maxit, nearest_Xt, nearest_Xg)
        ...
        real(rk), intent(in) :: tol
            !! Nonnegative absolute gradient or parameter-step tolerance; not internally validated.
    ...
    !! derivative with respect to arc length. At a zero tangent, physical
    !! gradients, second arc-length derivatives, and `dL` are returned as zero;
    !! no tangent direction is defined at that point.
    !!
    ...
    pure subroutine ansatz(this, ie, ig, Tgc, dTgc_dXg, dL, ngauss, d2Tgc_dXg2)
        class(nurbs_curve), intent(inout) :: this
    ...
    pure subroutine cmp_length(this, length, ngauss)
        ...
        real(rk) :: Xt, dXt_dXksi, denom, ddenom, wi, inv_denom2
        real(rk) :: dL, norm2_dXg, Sw_d, dSw_d, dXg_dXt_d
        integer :: ie, ig, j, d, idx, first, dim_, j0, j1, ngauss_, nelem
        logical :: rational
        ...
        do concurrent (ie = 1: nelem) &
            local(N, dN, Xt, dXt_dXksi, denom, ddenom, wi, inv_denom2, dL, norm2_dXg, &
                Sw_d, dSw_d, dXg_dXt_d, ig, j, d, idx, first, j0, j1) &
            reduce(+:length)
#endif
            dL = 0.0_rk
            do ig = 1, ngauss_
                ...
                if (rational) then
                    ...
                    if (abs(denom) > 0.0_rk) then
                        ...
                        do d = 1, dim_
                            ...
                            dXg_dXt_d = (dSw_d*denom - Sw_d*ddenom)*inv_denom2
    ...
    pure function compute_Xg_nurbs_1d(Xt, knot, degree, nc, ng, Xc, Wc, wrap_parameters) result(Xg)
        ...
        logical, intent(in), optional :: wrap_parameters
        real(rk), allocatable :: Xg(:,:)
        real(rk) :: N(0:degree), bw, wsum, wtol
        ...
        do i = 1, ng_
#else
        do concurrent (i = 1: ng_) local(N, wsum, bw, j, d, idx, first)
#endif
            do d = 1, dim_
                Xg(i,d) = 0.0_rk
            end do
    ...
    pure subroutine compute_d2Tgc_nurbs_1d_vector(Xt, knot, degree, nc, ng, Wc, d2Tgc, dTgc, Tgc, wrap_parameters)
        ...
        do i = 1, ng_
#else
        do concurrent (i = 1: ng_) local(N, dN, d2N, denom, ddenom, d2denom, wi, j, idx, first)
#endif
            call basis_bspline_der(map_parameter(Xt(i), knot, nc, degree, wrap_parameters_), &
    ...
    pure subroutine compute_dTgc_nurbs_1d_vector(Xt, knot, degree, nc, ng, Wc, dTgc, Tgc, wrap_parameters)
        ...
        do i = 1, ng_
#else
        do concurrent (i = 1: ng_) local(N, dN, denom, ddenom, wi, j, idx, first)
#endif
            call basis_bspline_der(map_parameter(Xt(i), knot, nc, degree, wrap_parameters_), &
    ...
    pure function compute_Tgc_nurbs_1d_vector(Xt, knot, degree, nc, ng, Wc, wrap_parameters) result(Tgc)
        ...
        do i = 1, ng_
#else
        do concurrent (i = 1: ng_) local(N, wsum, j, idx, first)
#endif
            call basis_bspline_der(map_parameter(Xt(i), knot, nc, degree, wrap_parameters_), &
                knot, nc, degree, 0, first, N)
            wsum = 0.0_rk
            do j = max(0, 1 - first), min(degree, nc - first)
                idx = first + j
                wsum = wsum + N(j)*Wc(idx)
            end do
            if (abs(wsum) > wtol) then
                do j = max(0, 1 - first), min(degree, nc - first)
                    idx = first + j
                    Tgc(i,idx) = N(j)*Wc(idx)/wsum
                end do
            end if
        end do
    end function
    ...
    !! mean weight one. In exact arithmetic exponentiation preserves strict
    !! positivity; overflow or underflow of the machine exponential is not
    !! checked. The damping is reduced geometrically and no trial-step
    !! acceptance or line search is performed.
    !!
    !! With residuals
    ...
    !! \(\rho_{old}-\rho\le\mathtt{tol}\max(1,\rho_{old})\); an objective
    !! increase therefore also stops a run with nonnegative `tol`. Reaching
    !! `maxit` is not reported as an error, and the last iterate is retained.
    !! Only `Xc`, `Wc`, and rational-state metadata are updated; cached `Xg` is
    !! not recomputed.
    !!
    ...
    !! coordinate, with `ndata>=nc`, `maxit>0`, `tol>=0`, and nonnegative
    !! regularization/damping values. These preconditions are not all validated.
    !! @endwarning
    !!
    ...
    pure subroutine lsq_fit_nurbs(this, Xt, Xdata, ndata, maxit, tol, lambda_xc, mu0, reg_logw)
        ...
        real(rk) :: cost_prev, cost_now, cost_sum, damp, denom, epss, ha, hb, resid, ta, tb
        ...
        this%Wc = exp(v-sum(v)/real(this%nc, rk))
        this%rational = max(maxval(this%Wc)-this%Wc(1),this%Wc(1)-minval(this%Wc)) > &
            32.0_rk*epsilon(1.0_rk)*maxval(this%Wc)
        ...
        do it = 1, maxit_
            ...
            do i = 1, ndata
                ...
                do k = 1, dim_
                    ...
                    do a = 0, this%degree
                        ia = first + a
                        if (ia < 1 .or. ia > this%nc) cycle
                        ha = N(a)*this%Wc(ia)*(this%Xc(ia,k) - Cg(k))/denom
                        Jrhs(ia,1) = Jrhs(ia,1) + ha*resid
                        do b = 0, this%degree
                            ib = first + b
                            if (ib < 1 .or. ib > this%nc) cycle
                            if (ia >= ib) then
                                hb = N(b)*this%Wc(ib)*(this%Xc(ib,k) - Cg(k))/denom
            ...
            if (size(delta_u,1) /= this%nc) then
                call this%err%set(&
                    code       = 108,&
                    severity   = 1,&
                    category   = "forcad_nurbs_curve",&
