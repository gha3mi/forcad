...
!! \]
!!
!! The degree-of-freedom map identifies order-zero controls paired by recorded
!! topology. Call `is_valid` before treating those pairs as geometrically
!! coincident. Higher continuity remains explicit as a sparse homogeneous system
!! \(\mathbf A\mathbf q=\mathbf0\), which can be eliminated, constrained with
!! multipliers, or supplied to another analysis package. For polynomial fields,
!! \(\mathbf q\) contains ordinary B-spline coefficients. For rational geometry,
!! apply the matrix to homogeneous controls \((w\mathbf P,w)\). A rational scalar
!! field with coefficients \(q_i\) uses weighted numerator controls \(w_iq_i\),
!! not the unweighted coefficient vector, while the denominator must satisfy the
!! same projective interface compatibility.
module forcad_multipatch_curve
    ...
    use forcad_multipatch, only: multipatch_chain_rule_coefficients, multipatch_connection, &
        multipatch_growth_capacity, multipatch_valid_reparameterization
    use forcad_nurbs_curve, only: nurbs_curve
    use forcad_utils, only: active_knot_multiplicity, active_span_count, basis_bspline_der_order_active, boundary_index, &
    ...
    !! `continuity=0` merges matching endpoint degrees of freedom; higher orders
    !! additionally appear in the sparse constraint matrix.
    !!
    ...
    contains
        ...
        procedure :: cmp_dof_map => cmp_dof_map_curve !!> Map unmerged control variables to compact shared \(C^0\) identifiers.
...
contains
    ...
    pure subroutine add_patch_curve(this, patch, id)
        ...
        type(nurbs_curve), intent(in) :: patch
            !! Curve patch whose diagnostic state must be clear.
    ...
    !> author: Seyed Ali Ghasemi
    !> license: BSD 3-Clause
    !> Build compact global identifiers for topologically shared endpoints.
    !!
    !! The input position is the unmerged index defined by
    !! [[nurbs_multipatch_curve:cmp_dof_offsets]]. Connections with continuity
    !! at least zero merge their paired boundary controls. Higher derivative
    !! conditions are not eliminated here; use `cmp_dof_constraint`. This method
    !! trusts recorded topology and does not call `is_valid`.
    !!
    ...
    pure function cmp_dof_map_curve(this) result(map)
        ...
        do i = 1, this%get_nconnection()
            associate(conn => this%connections(i))
                ...
                call boundary_index(this%patches(conn%get_patch_a())%get_nc(), conn%get_side_a(), .false., ia)
                call boundary_index(this%patches(conn%get_patch_b())%get_nc(), conn%get_side_b(), conn%is_reversed(), ib)
                do j = 1, size(ia)
    ...
    !> author: Seyed Ali Ghasemi
    !> license: BSD 3-Clause
    !> Check projective compatibility of rational endpoint weights.
    !!
    !! Compatible traces satisfy \(w_j^{(A)}=\lambda w_j^{(B)}\) within
    !! `tol`; `weight_scale` returns \(\lambda\). Polynomial traces
    !! use unit scale. This helper does not modify patch data.
    pure subroutine connection_weight_scale_curve(this, conn, tol, weight_scale, ok)
        ...
        call boundary_index(this%patches(pb)%get_nc(), conn%get_side_b(), conn%is_reversed(), ib)
    ...
    !! uses compact identifiers from [[nurbs_multipatch_curve:cmp_dof_map]].
    !!
    pure function cmp_elem_curve(this, shared) result(elem)
        class(nurbs_multipatch_curve), intent(in) :: this
            !! Multipatch curve.
        logical, intent(in), optional :: shared
            !! Select compact shared \(C^0\) numbering; default false.
    ...
    !!
    !! Validation covers patch diagnostics, patch/side ranges, continuity order,
    !! endpoint-control coincidence, transition jets, homogeneous derivative
    !! residuals, and rational projective weight compatibility. `tol` is used as
    !! one absolute threshold for physical coordinates and assembled homogeneous
    !! residuals; the default is `sqrt(epsilon(rk))`. Specifically, paired
    !! coordinates and each component of \(A\mathbf H\) are compared directly
    !! with `tol`. Weight proportionality uses
    !! `max(tol,128*epsilon(rk))*max(abs(wA),abs(lambda*wB))` for each pair.
