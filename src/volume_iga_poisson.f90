...
program volume_iga_poisson
   ...
   filename = "vtk/volume_iga_poisson"
   ...
   write(*,'(a,g0,",",g0,",",g0)') "Degree (dir1, dir2, dir3)                  : ", vol%get_degree(1), vol%get_degree(2), vol%get_degree(3)
   write(*,'(a,g0,"x",g0,"x",g0)') "Control net size (dir1 x dir2 x dir3)      : ", nc(1), nc(2), nc(3)
   write(*,"(a,g0)") "Total control points                       : ", nct
   write(*,"(a,g0)") "Number of elements                         : ", nelem
   write(*,"(a,g0)") "Degrees of freedom (DoFs)                  : ", ndof
   write(*,'(a,g0," x ",g0," x ",g0)') "Domain size (L1, L2, L3)                   : ", L(1), L(2), L(3)
   write(*,'(a,g0,",",g0,",",g0)') "Mode numbers (m1, m2, m3)                  : ", m(1), m(2), m(3)
   write(*,'(a,g0,",",g0,",",g0)') "Knot insertion (dir1, dir2, dir3)          : ", ki(1), ki(2), ki(3)
   write(*,'(a,g0,"x",g0,"x",g0)') "Visualization resolution (res1, res2, res3): ", res(1), res(2), res(3)
   ...
   call vol%export_Xth_in_Xg(trim(filename)//"_Xth.vtk", res=maxval(res))
   call vol%show(trim(filename)//".vtk", trim(filename)//"_interp.vtk", trim(filename)//"_Xth.vtk")
   ...
   write(*,"(a,1pe11.4)") "L2 error norm                        = ", sqrt(l2_error)
   write(*,"(a)") trim(filename)//".vtk, "//trim(filename)//"_interp.vtk, and "//trim(filename)//"_Xth.vtk exported"
...
end program volume_iga_poisson
